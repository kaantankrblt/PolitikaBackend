...
def fetch_top_news_for_countries(countries):
    ...
    for country in countries:
        print(f"Fetching financial news for {country}")
        country_news = fetch_news(f"Financial news in {country}")
...
def update_firestore(news_list):
    ...
    existing_news = db.collection("Financial_news").get()
    ...
    for news_item in news_list:
        if news_item["link"] in existing_news_dict:
            ...
            # Add new news
            db.collection("Financial_news").add(news_item)

    # Fetch all news again
    all_news = db.collection("Financial_news").get()
...
# Save to JSON file
with open("Financial_news.json", "w") as json_file:
