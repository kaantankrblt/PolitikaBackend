...
def fetch_top_news_for_countries(countries):
    ...
    for country in countries:
        print(f"Fetching political news for {country}")
        country_news = fetch_news(f"{country} Politics")
...
def update_firestore(news_list):
    ...
    existing_news = db.collection("Political_news").get()
    ...
    for news_item in news_list:
        if news_item["link"] in existing_news_dict:
            ...
            # Add new news
            db.collection("Political_news").add(news_item)

    # Fetch all news again
    all_news = db.collection("Political_news").get()
...
political_news = fetch_top_news_for_countries(countries)

# Update Firestore
update_firestore(political_news)

driver.quit()

# Save to JSON file
with open("Political_news.json", "w") as json_file:
    json.dump(political_news, json_file, indent=4)
