...
def update_firestore(news_list):
    ...
    existing_news = db.collection("top_world_news").get()
    ...
    for news_item in news_list:
        if news_item["link"] in existing_news_dict:
            ...
            # Add new news
            db.collection("top_world_news").add(news_item)

    # Fetch all news again
    all_news = db.collection("top_world_news").get()
    ...
    for doc in sorted_news[10:]:
        doc.reference.delete()


# Fetch top world news
print("Fetching top world news")
top_news = fetch_top_world_news()
...
# Save to JSON file
with open("top_world_news.json", "w") as json_file:
    json.dump(top_news, json_file, indent=4)

# Save to Excel file
df = pd.DataFrame(top_news)
df.to_excel("top_world_news.xlsx", index=False)

print(
    "Top world news has been processed and uploaded to Firebase Firestore. News data has been saved to top_world_news.json and top_world_news.xlsx"
)
